    ...
    <v-row class="d-flex justify-center mb-9">
      <v-col cols="12" sm="12" md="12" lg="12">
        <v-data-table
          ...
          <template v-slot:top>
            <v-toolbar flat class="mt-2">
              <v-toolbar-title>
                جدول تاريخ الصلاحية لمواد الصيدلية
              </v-toolbar-title>
          ...
          <template v-slot:item="{ item }">
            <tr>
              ...
              <td class="text-center font-weight-black">
                <h4
                  style="direction: ltr; color: #ff9800"
                  v-if="item.expaired > moment().format('YYYY-MM-DD')">
                  {{ item.expaired }}
                </h4>
                <h4 style="direction: ltr; color: red" v-else>
                  {{ item.expaired }}
                </h4>
              </td>
              <td class="text-center font-weight-black">
        ...
        <PopDelete
          ...
          v-on:deleteMedicine="deleteMedicine()" />
        <PopEdit
          :value="pop_edit"
          :medicine="data_editor"
...
<script>
  import PopDelete from "@/components/pharmacies/PopDelete.vue";
  import PopEdit from "@/components/pharmacies/PopEdit.vue";
  import PopViewDetails from "@/components/pharmacies/PopViewDetails";
  export default {
    ...
    computed: {
      medicine_query: {
        set(val) {
          this.$store.state.pharmacies.medicine_query = val;
        },
        get() {
          return this.$store.state.pharmacies.medicine_query;
        },
      },
      pageCount() {
        return this.$store.state.pharmacies.pageCount;
      },

      expired_medication() {
        return this.$store.state.pharmacies.expired_medication;
      },
      loading() {
      ...
      params: {
        set(val) {
          this.$store.state.pharmacies.params = val;
        },
        get() {
          return this.$store.state.pharmacies.params;
        },
      },
    },
    methods: {
      ...
      get_medicines() {
        ...
        this.params = par;
        this.$store.dispatch("pharmacies/get_medicines");
      },
      query_change() {
      ...
      // حذف دواء من الصيدلية
      deleteMedicine() {
        let data = {};
        data["id"] = this.data_deleted;
        this.$store.dispatch("pharmacies/delete_medicine", data).then(() => {
          this.pop_delete = false;
          this.data_deleted = null;
        });
      },
      reset_table() {
        this.$store.dispatch("pharmacies/reset_table_exp");
        this.get_medicines();
      },
    },
    watch: {
